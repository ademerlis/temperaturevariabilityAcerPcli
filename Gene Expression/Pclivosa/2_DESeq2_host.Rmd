...
```{r}
#read in counts
counts = readxl::read_xlsx("1_bioinformatics/allcounts_pcli.xlsx") #alignment to Avila-Magana et al. 2021 transcriptome 
...
min(totalCountshost)  #22808
max(totalCountshost) #1919760
```
...
#this corresponds to 4 samples with the lowest counts
as.data.frame(totalCountshost) %>% 
  arrange(totalCountshost)
...
#code by group
pdf("Host_PCoA_Treatment_Genotype.pdf")
plot(scores[,1], scores[,2], pch=c(15,17,25)[as.numeric(as.factor(conditions$Genotype))], col=c("#00CCCC", "grey","#FF3333")[as.numeric(as.factor(conditions$Treatment))], xlab="PC1: 15%", ylab="PC2: 6%", main="Treatment")
...
dispersion_genotype <- betadisper(dist_mat, group = conditions$Geno_Treatment)
...
save(vsd,design,file="Rdata_files/host/vsd.RData")
```
...
# saving all models
save(dds,file="Rdata_files/host/realModels_Pcli.RData")
```
...
## import gene names
```{r}
load("Rdata_files/host/realModels_Pcli.RData")
library(DESeq2)
library(tidyverse)

read.table(file = "1_bioinformatics/Pclivosa_iso2geneName.tab",
                          sep = "\t",
                          quote="", fill=FALSE) %>%
...
length(degs_Treatment_Variable_vs_Control) #21
...
length(degs_Treatment_Variable_vs_Control_L2FC1) #10
...
length(degs_Treatment_Variable_vs_Initial) #141
...
length(degs_Treatment_Variable_vs_Initial_L2FC1) #64
...
length(degs_Treatment_Control_vs_Initial) #132
...
length(degs_Treatment_Control_vs_Initial_L2FC1) #61
...
rbind(variable_vs_control, variable_vs_initial, control_vs_initial) %>% 
  write_csv("host_all_contrasts_annotDEGs.csv")

save(Treatment,Treatment_Control_vs_Initial,Treatment_Variable_vs_Initial,Treatment_Variable_vs_Control, degs_Treatment_Control_vs_Initial,degs_Treatment_Variable_vs_Initial, degs_Treatment_Variable_vs_Control, file="Rdata_files/host/pvals.RData")
```
...
ggvenn(pairwise_LFC1) + 
  scale_fill_manual(values = c("#ca0020", "#0571b0", "#f4a582"))
ggsave("Host_VennDiagram_L2FC1.pdf")
```
...
write.csv(Control_vs_Initial.fc,file="Host_Control_vs_Initial_fc.csv",row.names=F,quote=F)
save(Control_vs_Initial.fc,file="Rdata_files/host/Control_vs_Initial.fc.RData")
...
write.csv(Control_vs_Initial.p,file="Host_Control_vs_Initial_lpv.csv",row.names=F,quote=F)
save(Control_vs_Initial.p,file="Rdata_files/host/Control_vs_Initial_lpv.RData")
...
write.csv(Variable_vs_Initial.fc,file="Host_Variable_vs_Initial_fc.csv",row.names=F,quote=F)
save(Variable_vs_Initial.fc,file="Rdata_files/host/Variable_vs_Initial_fc.RData")
...
write.csv(Variable_vs_Initial.p,file="Host_Variable_vs_Initial_lpv.csv",row.names=F,quote=F)
save(Variable_vs_Initial.p,file="Rdata_files/host/Variable_vs_Initial_lpv.RData")
...
write.csv(Variable_vs_Control.fc,file="Host_Variable_vs_Control_fc.csv",row.names=F,quote=F)
save(Variable_vs_Control.fc,file="Rdata_files/host/Variable_vs_Control_fc.RData")
...
write.csv(Variable_vs_Control.p,file="Host_Variable_vs_Control_lpv.csv",row.names=F,quote=F)
save(Variable_vs_Control.p,file="Rdata_files/host/Variable_vs_Control_lpv.RData")
```
...
```{r}
all_annotDEGs <- read_csv("results_csv/host/host_all_contrasts_annotDEGs.csv")
load("Rdata_files/host/pvals.RData")
...
length(all_intersections$`Variable vs. Initial`) #47
...
all_genes <- c(genes_V, genes_VvC, genes_VvI) %>% 
...
#write_csv(Variable_genes_annot, "Pclivosa_variable_DEGs_annotated.csv")
...
# write_csv(top20genes, "Pcli_host_top20DEGs.csv")
# 
# write_csv(top10genes, "Pcli_host_top10DEGs.csv")
```
