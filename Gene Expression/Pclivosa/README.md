...
The folder **1_bioinformatics** contains [tagSeq_processing_README.txt](https://github.com/ademerlis/temperaturevariabilityAcerPcli/blob/main/Gene%20Expression/Pclivosa/1_bioinformatics/tagSeq_processing_README.txt), which has the code Dr. Michael Studivan used to analyze the Pcli samples on the FAU HPC.

The perl (.pl) scripts that Michael used are hosted on his GitHub repository [here](https://github.com/mstudiva/tag-based_RNAseq). 

The pipeline is as follows:
1. Download raw sequences (reads) from BaseSpace using CLI.
2. Count number of reads in each sample using countreads.pl
3. Use tagseq_clipper.pl and cutadapt to trim Illumina and 3' Tag-Seq specific adapters.
4. Count number of reads in trimmed samples using countreads_trim.pl
5. Create annotated transcriptome from the transcriptome assembly generated by [Avila-Magana et al. 2021](https://doi.org/10.5061/dryad.k3j9kd57b). Code for generating this annotated transcriptome is hosted on Michael's GitHub for *P.clivosa* [here](http://github.com/mstudiva/Pseudodiploria-clivosa-annotated-transcriptome) and for *Breviolum* [here](https://github.com/mstudiva/Breviolum-annotated-transcriptome).
6. Create reference genome index using bowtie2. Contatenate host transcriptome (*P.clivosa*) with symbiont transcriptome (*Breviolum*).
7. Map reads to host/symbiont transcriptomes with bowtie2.
8. Count number of mapped reads for mapping efficiency using countreads_align.pl.
9. Generate read counts per gene using samtools.
