---
title: "samplesizes"
author: "allyson_demerlis"
date: "2023-02-22"
output: html_document
---

```{r}
metadata <- readxl::read_xlsx("../physiology/metadata.xlsx")

library(tidyverse)
library(janitor)
```


```{r}

#what are all the tissue sampling time points?
# 2022-03-22, 2022-03-30, 2022-04-20, 2022-04-22, 2022-06-01
metadata %>% 
  drop_na(tissue_sampling_date_1) %>% 
  group_by(Species,Treatment, tissue_sampling_date_1) %>% 
  summarise(count = n()) %>% 
  ggplot(., aes(x=tissue_sampling_date_1, y=count, fill = Treatment)) +
  geom_col() +
  facet_wrap(~Species, scales = "free")  +
  scale_fill_manual(labels=c("Control", "Variable"), values = c( "#60DBDB", "#F54A34")) +
  theme_classic()
```

```{r}
#drop the last time point so it matches the temp-treatment figure (which only has 3 time points before CBASS one)

metadata %>% 
  drop_na(tissue_sampling_date_1) %>% 
  filter(tissue_sampling_date_1 < "2022-04-21") %>% 
  group_by(Species,Treatment, tissue_sampling_date_1) %>% 
  summarise(count = n()) %>% 
  ggplot(., aes(x=tissue_sampling_date_1, y=count, fill = Treatment)) +
  geom_col(position=position_dodge()) +
  facet_wrap(~Species, scales = "free")  +
  scale_fill_manual(labels=c("Control", "Variable"), values = c( "#60DBDB", "#F54A34")) +
  theme_classic()

```

