...
# saving all models
save(dds,file="Rdata_files/host/realModels_Acer.RData")
```
...
## import gene names
```{r}
load("Rdata_files/host/realModels_Acer.RData")
library(DESeq2)
library(tidyverse)

read.table(file = "1_bioinformatics/Acervicornis_iso2geneName.tab",
                          sep = "\t",
                          quote="", fill=FALSE) %>%
  rename(gene = V1,
...
```{r}
all_annotDEGs <- read_csv("results_csv/host/all_contrasts_annotDEGs.csv")
...
length(all_intersections$`Variable vs. Control`) #106

genes_VvC <- all_intersections$`Variable vs. Control`

length(all_intersections$`Variable vs. Initial`) #400 

genes_VvI <- all_intersections$`Variable vs. Initial`

length( all_intersections$`Variable vs. Initial:Variable vs. Control`) #77 
...
#write_csv(Variable_genes_annot, "host_variable_DEGs_annotated.csv")
...
# write_csv(top20genes, "Acer_host_top20DEGs.csv")
# 
# write_csv(top10genes, "Acer_host_top10DEGs.csv")
```
