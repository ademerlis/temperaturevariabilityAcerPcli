...
## 2b. compare to when tank 8 and treated are combined 
```{r}
treatment_normalized_bothspecies_notank8separate <- ipam_tidy_data %>% 
  dplyr::filter(!Treatment_period == "CBASS") %>%
  drop_na(Colony) %>% #this is because there are some corals that never matched up to IPAM values so they're just NA
  dplyr::select(Date, Colony, Puck, Tank, fvfm, Treatment, Species)  %>% 
    mutate(Treatment = case_when(Tank == "1" ~ "Untreated",
...
```{r}
treatment_normalized_bothspecies_notank8separate%>% 
  ggplot(., aes(x=Treatment, y= fvfm_loss_norm, color= Treatment)) +
  geom_boxplot(outlier.shape = NA) + 
  geom_point(position = position_jitterdodge(), alpha=0.3) +
  theme_classic() + 
...
Change to factors
```{r}
str(treatment_normalized_bothspecies_notank8separate)
#make factors: Colony, Puck, Tank, Treatment, Species

treatment_normalized_bothspecies_notank8separate %>% 
  mutate_at(vars(Colony, Puck, Tank, Treatment, Species), factor) -> treatment_normalized_bothspecies_notank8separate
```

Create linear model to test normality of residuals and homogeneity of variance. Remove outliers
```{r}
treat_model <- lm(fvfm_loss_norm ~ Treatment*Species*Colony + Tank, data = treatment_normalized_bothspecies_notank8separate)
treat_model_metrics <- augment(treat_model)
plot(treat_model)
...
#remove outliers
treatment_normalized_bothspecies_notank8separate <- treatment_normalized_bothspecies_notank8separate[!rownames(treatment_normalized_bothspecies_notank8separate) %in% treat_outliers$.rownames,]
```

Run 2-way ANOVA
```{r}
aov(fvfm_loss_norm ~ Treatment*Species, data = treatment_normalized_bothspecies_notank8separate)

summary(aov(fvfm_loss_norm ~ Treatment*Species, data = treatment_normalized_bothspecies_notank8separate))

#capture.output(summary(aov(fvfm_loss_norm ~ Treatment*Species, data = treatment_normalized_bothspecies)), file = "fvfm_loss_norm_bothspecies_anova_table.csv")


TukeyHSD(aov(fvfm_loss_norm ~ Treatment*Species, data = treatment_normalized_bothspecies_notank8separate))
         
#capture.output(TukeyHSD(aov(fvfm_loss_norm ~ Treatment*Species, data = treatment_normalized_bothspecies)), file = "fvfm_loss_norm_bothspecies_anova_tukey.csv")
```


## 2c. Reduction of Fv/Fm over 28-d Treatment, normalized to initial measurement (genotypes separated)
```{r, warning = F}
treatment_normalized_bothspecies%>% 
...
Run 3-way ANOVA
```{r}
aov(fvfm_loss_norm ~ Treatment*Species*Colony, data = treatment_normalized_bothspecies_notank8separate)

summary(aov(fvfm_loss_norm ~ Treatment*Species*Colony, data = treatment_normalized_bothspecies_notank8separate))
...
TukeyHSD(aov(fvfm_loss_norm ~ Treatment*Species*Colony, data = treatment_normalized_bothspecies_notank8separate))
