...
First plot the timeline of when corals were removed.
```{r}
tidy_metadata %>%
...
slowburn_metadata %>% 
  ...
  mutate(days_to_removed = end_date - start_date) %>% 
  filter(!days_to_removed == "NA days") %>% 
  group_by(Species,Treatment, Removed_Date, mortality_date, Reason_removed, slowburn) %>% 
  summarise(count = n()) %>% 
  mutate(Reason_removed = case_when(Reason_removed == "dead" ~ "RTL",
                             Reason_removed == "RTL" ~ "RTL",
...
#figure out reason for mortality in the ones that have mortality date -- check photos and notes
```
