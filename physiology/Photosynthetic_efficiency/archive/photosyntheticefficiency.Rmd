...
#11 rows come up with NA for their metadata. this is because they were not annotated when they were PAMed, so we  don't know  their identities. so drop them from the dataset
```
...
### normalized PAM data (initial values all start at 1) across 28-d treatment
...
ipam_tidy_data %>% 
  ...
  mutate(normalized_fvfm_T1 = case_when(Species == "Acervicornis" ~ (`2022-04-06`/`2022-03-16`),
   ...
   mutate(normalized_fvfm_T2 = case_when(Species == "Acervicornis" ~ (`2022-04-20`/`2022-03-16`),
                                  Species == "Pclivosa" ~ (`2022-04-20`/`2022-03-21`))) -> ipam_normalized_data

# now removing the initial data points for statistical tests, since they are all '1'
ipam_acer_sub <- ipam_normalized_data %>% 
  select(!normalized_fvfm_T0) %>% 
  filter(Species == "Acervicornis")
ipam_pcli_sub <-  ipam_normalized_data %>% 
  select(!normalized_fvfm_T0) %>% 
  filter(Species == "Pclivosa")
```


change time to numeric as days in treatment 
