...
make table for WGCNA
```{r}
ipam_tidy_CBASS_treatments %>% 
  select(Puck:fvfm, CBASS_temp) %>% 
  write_csv("cbass_fvfm_forwgcna.csv")
```
...
# ED25s are not significantly different
```
...
Acer_coeff %>% 
  ...
  dplyr::rename(ED50 = `DRC_Acer_colony_treatment$coefficients`) %>% 
  separate(coeff_colony_treatment, into=c("ed50", "Colony", "Treatment"), sep = "[:.]") -> Acer_ED50s

#write_csv(Acer_ED50s, "Acer_ED50s.csv")
...
Pcli_coeff %>% 
  rownames_to_column(var = "coeff_colony_treatment") %>% 
  filter(grepl("ed50", coeff_colony_treatment)) %>% 
  dplyr::rename(ED50 = `DRC_Pcli_colony_treatment$coefficients`) %>% 
  separate(coeff_colony_treatment, into=c("ed50", "Colony", "Treatment"), sep = "[:.]") -> Pcli_ED50s

#write_csv(Pcli_ED50s, "Pcli_ED50s.csv")
...
# Converting temperature to Degree Heating Minutes
