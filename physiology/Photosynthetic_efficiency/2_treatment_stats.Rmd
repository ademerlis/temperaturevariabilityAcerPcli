...
## 2a. percent change boxplot (for publication)
```{r}
...
treatment_normalized_bothspecies %>% 
  filter(Species == "Acervicornis") -> Acer_fvfm_norm

treatment_normalized_bothspecies %>% 
  filter(Species == "Pclivosa") -> Pcli_fvfm_norm

Pcli_fvfm_norm %>% 
  drop_na(`2022-04-20`) -> Pcli_fvfm_norm

Acer_model <- lmer(fvfm_loss_norm ~ Treatment*Colony + (1|Tank), data = Acer_fvfm_norm)
summary(Acer_model)
anova(Acer_model)

Pcli_model <- lmer(fvfm_loss_norm ~ Treatment*Colony + (1|Tank), data = Pcli_fvfm_norm)
summary(Pcli_model)
anova(Pcli_model)
```
...
leveneTest(x ~ Treatment*Colony, data=Acer_fvfm_norm, center=mean) # formal statistical test for homogenity of variance #marginally significant
...
# removal of one outlier made the variance more significant, so leaving it as is
...
leveneTest(x ~ Treatment*Colony, data=Pcli_fvfm_norm, center=mean) # formal statistical test for homogeneity of variance #not significant
qqnorm(x); qqline(x) # qq plot to check for normal distribution of residuals
hist(x) # histogram of residuals to check for normal distribution of residuals
shapiro.test(x) # formal statistical test 
# Perform Bonferroni outlier test
bonf_outliers <- outlierTest(Pcli_model, cutoff = Inf, n.max = Inf)

# View all potential outliers identified with Bonferroni correction
bonf_outliers #row 12

# Indices of the rows to remove
rows_to_remove <- c(12)

# Remove these rows
Pcli_clean <- Pcli_fvfm_norm[-rows_to_remove, ]

Pcli_model_2 <- lmer(fvfm_loss_norm ~ Treatment*Colony + (1|Tank), data = Pcli_clean)
x = residuals(Pcli_model_2)
plot(fitted(Pcli_model_2), x) 
leveneTest(x ~ Treatment*Colony, data=Pcli_clean, center=mean) # formal statistical test for homogenity of variance 
qqnorm(x); qqline(x) # qq plot to check for normal distribution of residuals
hist(x) # histogram of residuals to check for normal distribution of residuals
shapiro.test(x)

Anova(Pcli_model_2, type = "II", test.statistic = "F") #you need to manually specify Satterthwaite's method, because otherwise it will give you the Wald/Chi-squared approximation and that is only appropriate for binomial or poisson distributions. Satterthwaite's method is for normal/gaussian

capture.output(Anova(Pcli_model_2, type = "II", test.statistic = "F"), file = "Pcli_model_ANOVA.csv")
```
...
```{r}
#colony + treatment
Acer_emms <- emmeans(Acer_model, pairwise ~ Colony*Treatment)

pairs(Acer_emms) 
capture.output(pairs(Acer_emms), file="Acer_percentchange_treatment_colony_emmeans.csv")
```

Pcli
```{r}
#colony + treatment
Pcli_emms <- emmeans(Pcli_model_2, pairwise ~ Colony*Treatment)

pairs(Pcli_emms) 
capture.output(pairs(Pcli_emms), file="Pcli_percentchange_treatment_colony_emmeans.csv")
```


## 2b. Linear over time (for publication) --  Normalized to 1
```{r, warning = FALSE}
...
```{r}
plot_grid(p1, p2, p3, rel_widths = c(1, 1, 2), nrow=1)
ggsave("treatment_normfvfm_boxplot_linear.pdf", width = 12, height = 5)
```
...
## 2c. Percent reduction of Fv/Fm over 28-d Treatment, normalized to initial measurement (all genotypes combined)
```{r, warning = F}
p1<-treatment_normalized_bothspecies%>% 
  ggplot(., aes(x=Species, y= fvfm_loss_norm, fill= Treatment)) +
  geom_boxplot() + 
...
## 2d. Percent reduction of Fv/Fm over 28-d Treatment, normalized to initial measurement (genotypes separated)
```{r, warning = F}
